...
%Si la photosynthèse est le processus majeur d'assimilation du \coo.
...
%\footnote{Production de matières organiques à l'aide de composés minéraux simples.}
}.
...
%'estimation de ce flux difficile.
...
%\si{\gram~de matières sèches \per\square\metre\per\year}. 
...
%Le niveau de la nappe d'eau en contraignant l'accès à l'oxygène contraint également la respiration aérobie de façon importante \citep{blodau2002}.
...
%Par ailleurs la végétation, en parallèle de son activité photosynthétique, libère des substrats, les exsudats racinaires, affectant également la respiration du sol.
...
\begin{equation}
%%BCNE=\overbrace{PPB - Re}^{ENE}  - F_{COD} - F_{COP} - F_{CH_{4}} - F_{CID} - F_{COV} - F_{CO}
%\frac{dC}{dt}=
...
%\vspace*{.2cm}
%\item \textcolor{black}{F$_{CID}$ : Flux de Carbone Inorganique Dissous}
%\item \textcolor{black}{F$_{COV}$ : Flux de Composés Organique Volatils}
%\item \textcolor{black}{F$_{CO}$ : Flux de Monoxyde de Carbone}
\end{itemize}
